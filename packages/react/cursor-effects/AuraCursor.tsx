...
export const AuraCursor = React.forwardRef<any, any>(({ className, ...props }, ref) => {
        ...
        const r = localRef.current?.getBoundingClientRect(); if (!r) return;
        const x = e.clientX - r.left, y = e.clientY - r.top;
        setPos({ x, y });
        setTrail(t => [...t.slice(-15), { x, y, id: Date.now() }]);
        };
        return (
        <div ref={handleRef} {...props} onMouseMove={handleMove} className={cn("relative w-full max-w-2xl h-64 rounded-2xl overflow-hidden bg-zinc-950 border border-white/8 cursor-none", className)}>
